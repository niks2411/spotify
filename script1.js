...
async function getsongs() {
    let a = await fetch("http://127.0.0.1:5500/"); // Updated to fetch from the root folder
    ...
    for (let index = 0; index < as.length; index++) {
        ...
        if (element.href.endsWith(".mp3")) {
            songs.push(element.href.split("/")[1]); // Just split by '/' as the songs are in the root folder
        }
    }
    return songs;
}
...
const playmusic = (track, pause = false) => {
    currentsong.src = "/" + track; // Update the path to the root folder where songs are now located
    if (!pause) {
        currentsong.play();
        play.src = "pause.svg";
    }
